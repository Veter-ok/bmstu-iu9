...
(define (interpret program stack)
  (let ((xs '((+ +) (- -) (* *) (/ /) (mod remainder))) (defines '()) (vars '()))
    (define (find label program depth)
      (cond ((null? program) '())
            ((equal? (car program) label) (if (= depth 0) (cdr program) (find label (cdr program) (- depth 1))))
            ((equal? (car program) 'if) (find label (cdr program) (+ depth 1)))
            (else (find label (cdr program) depth))))
    (define (find-case program value)
      (cond ((null? program) '())
            ((and (pair? program) (equal? (car program) 'case) (equal? (cadr program) value)) (cddr program))
            (else (find-case (cdr program) value))))
    (define (skip-to-else-or-endif program depth)
      (cond
        ((null? program) '())
        ((equal? (car program) 'if) (skip-to-else-or-endif (cdr program) (+ depth 1))) 
        ((equal? (car program) 'else) (if (= depth 0) (cdr program) (skip-to-else-or-endif (cdr program) depth)))
        ((equal? (car program) 'endif) 
         (if (= depth 0) (cdr program) (skip-to-else-or-endif (cdr program) (- depth 1))))
        (else (skip-to-else-or-endif (cdr program) depth))))
    ...
    (define (machine program stack)
      (cond ((null? program) stack)
            ...
            ((assoc (car program) vars) (machine (cdr program) (cons (cdr (assoc (car program) vars)) stack)))
            ((equal? (car program) 'define) (machine (add-define (cddr program) (cadr program) '()) stack))
            ((equal? (car program) 'exit) (machine (find 'end program 0) stack))
            ((equal? (car program) 'defvar) (set! vars (cons (cons (cadr program) (caddr program)) vars)) (machine (cdddr program) stack))
            ((equal? (car program) 'set) (set-cdr! (assoc (cadr program) vars) (car stack)) (machine (cdddr program) (cdr stack)))
            ...
            ((equal? (car program) 'rot) (machine (cdr program) (cons (caddr stack) (append (list (cadr stack) (car stack)) (cdddr stack)))))
            ((equal? (car program) 'depth) (machine (cdr program) (cons (length stack) stack)))
            ((equal? (car program) 'if)
             (if (= (car stack) 0)
                 (machine (skip-to-else-or-endif (cdr program) 0) (cdr stack))
                 (machine (cdr program) (cdr stack))))
            ((equal? (car program) 'else) (machine (find 'endif (cdr program) 0) stack))
            ((equal? (car program) 'switch)
             (machine (or (find-case (cdr program) (car stack)) (find 'endswitch (cdr program) 0)) (cdr stack)))
            ((equal? (car program) 'case) (machine (cddr program) stack))
            ((equal? (car program) 'exitcase) (machine (find 'endswitch (cdr program) 0) stack))
            (else (machine (cdr program) stack))))
    (machine (vector->list program) stack)))

(define the-tests
  (list  (test (interpret #(and) (quote (-1 -1))) (-1))
         (test (interpret #(not) (quote (0))) (-1))
         (test (interpret #(not) (quote (-1))) (0))
         (test (interpret #(not) (quote (77))) (0))
         (test (interpret #(rot) (quote (2 3 5 7 11))) (5 3 2 7 11))
